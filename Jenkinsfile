    ...
    stage('Build') {
      ...
      post {
        always {
          ...
          hipchatSend color: 'YELLOW', credentialId: 'HIPCHAT', message: '"${env.JOB_NAME} ${env.BUILD_NUMBER} status: ${currentBuild.result} (Open)"', notify: true, room: 'Next-Level', sendAs: '', server: '', textFormat: true, v2enabled: true
        }
